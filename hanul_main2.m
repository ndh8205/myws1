...
% 
%         % 자세 제어기 실행 (attitude_Control_Hz 주기로)
...
% 
%         final_cmd(2:5) = LIMIT2(final_cmd(2:5),deg2rad(-5),deg2rad(5));
% 
%         % 동역학 적분 - 업데이트된 파라미터 전달
...
% % MRP 위치 변화 시각화
% subplot(2,2,2);
% % 여기서는 params 구조체에서 MRP 위치를 추출해 저장해야 함
% % MRP_history가 이미 저장되어 있다고 가정
...
%     grid on;
% else
%     text(0.5, 0.5, 'MRP 데이터 없음', 'HorizontalAlignment', 'center');
% end
% 
% % αtot와 φA 시각화
% subplot(2,2,3);
% % alpha_tot_history와 phi_A_history가 이미 저장되어 있다고 가정
...
%     grid on;
% else
%     text(0.5, 0.5, '각도 데이터 없음', 'HorizontalAlignment', 'center');
% end
% 
% % 공력 힘과 모멘트 시각화 
% subplot(2,2,4);
% % F_aero_history와 M_aero_history가 이미 저장되어 있다고 가정
...
%     grid on;
% else
%     text(0.5, 0.5, '공력 데이터 없음', 'HorizontalAlignment', 'center');
% end
% 
% % moment_coupling 시각화 (메인 코드 맨 끝에 추가)
% figure(6);
% subplot(2,1,1);
...
% % 카나드 제어 입력
% subplot(2, 1, 1);
% plot(t, rad2deg(control_input_history(2:5, :)));
% xlabel('Time [s]');
...
% % RCS 추력기 제어 입력
% subplot(2, 1, 2);
% stairs(t, control_input_history(6:9, :)');
% xlabel('Time [s]');

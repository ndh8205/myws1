function [xdot, Thrust_mass_vec, alpha_tot, phi_A, F_b, M_b, moment_coupling] = vehicle_dynamics_HANul_pid(X, U, Qk, dt, params, t)
    ...
    % 제어 명령 벡터 분해 (8x1 벡터)
    % U의 형식: [메인엔진, 카나드1, 카나드2, 카나드3, 카나드4, 추력기1, 추력기2, 추력기3, 추력기4]
    main_engine_state = U(1); % 메인엔진 점화 명령 (1개)
    canard_angles = U(2:5);   % 카나드 각도 명령 (4개)
    thruster_cmds = U(6:9);   % 추력기 명령 (4개)
    ...
    % 추력 벡터 (바디 좌표계)
    F_T_body = [ main_engine_state * current_thrust; 0; 0 ];

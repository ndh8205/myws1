...
% end
function r = radiusByShape(shape, x, R, L, param)
% radiusByShape: 주어진 형상 타입에 따라 x 위치에서의 반경 계산
% 입력:
%   shape: 형상 이름 (문자열, 예: 'conical', 'ogive')
%   x:     반경을 계산할 축 방향 위치 (스칼라 또는 벡터)
%   R:     기준 반경 (보통 끝 반경 또는 반경 차이)
%   L:     기준 길이
%   param: 형상 파라미터 (해당 형상에만 사용)
% 출력:
%   r:     계산된 반경 (x와 동일한 크기)

% 입력값 유효성 검사 및 기본값 설정
if abs(L) < eps
    r = zeros(size(x)); % 길이가 0이면 반경도 0
    % fprintf('경고: radiusByShape - 기준 길이 L이 0에 가깝습니다.\n'); % 필요시 경고 추가
    return;
end
if isnan(R) || R < 0, R = 0; end % 유효하지 않은 반경은 0으로
if isnan(L) || L <= 0, L = eps; end % 유효하지 않은 길이는 작은 양수로
if isnan(param), param = 0; end % 유효하지 않은 파라미터는 0으로

r = zeros(size(x)); % 결과 배열 초기화 (기본값 0)

% 계산용 x값 준비: 입력 x의 범위를 [0, L]로 제한하고 유효 인덱스 저장
if isscalar(x)
    if x < -eps || x > L + eps % 유효 범위 밖이면 0 반환 (위에서 처리됨)
        return;
    end
    x_calc = max(0, min(x, L));
    valid_indices = true;
else
    valid_indices = (x >= -eps & x <= L + eps);
    x_calc = x;
    x_calc(x_calc < 0) = 0;
    x_calc(x_calc > L) = L;
    % 유효하지 않은 인덱스에 대해서는 계산하지 않음 (결과는 0 유지)
    x_calc = x_calc(valid_indices);
    if isempty(x_calc), return; end % 계산할 유효한 x가 없으면 종료
end

r_calc = zeros(size(x_calc)); % 계산 결과 임시 저장

try % 계산 중 발생할 수 있는 수학적 오류 처리
    switch lower(shape) % 소문자로 변환하여 비교
        case 'conical'
            r_calc = R .* x_calc ./ L;

        case 'ogive'
            MINFEATURE_OGIVE = 1e-5; % 매우 작은 param 값 처리 기준
            if abs(param) < MINFEATURE_OGIVE
                r_calc = R .* x_calc ./ L; % param=0 이면 코니컬
            else
                % Java의 getRadius(Ogive) 구현과 동일한 공식 사용
                % 중간 계산값 rho_sq 대신 직접 계산
                val_R_num = (L^2 + R^2) * (((2 - param) * L)^2 + (param * R)^2);
                val_R_den = 4 * (param * R)^2;

                if abs(val_R_den) < eps % 분모 0 방지
                    fprintf('경고: Ogive 계산 중 분모 0 발생 (param=%.2f, R=%.2f). 코니컬로 대체.\n', param, R);
                    r_calc = R .* x_calc ./ L;
                else
                    ogive_R = sqrt(max(0, val_R_num / val_R_den)); % 원의 반경 (음수 방지)
                    ogive_L_param = L / param; % 분모 0은 위에서 처리됨

                    val_y0_sq = ogive_R^2 - ogive_L_param^2;
                    if val_y0_sq < 0
                         fprintf('경고: Ogive 계산 중 y0^2 음수 발생 (param=%.2f, L=%.2f, R=%.2f). 코니컬로 대체.\n', param, L, R);
                         r_calc = R .* x_calc ./ L;
                    else
                         y0 = sqrt(val_y0_sq); % 중심 오프셋
                         val_r_sq = ogive_R^2 - (ogive_L_param - x_calc).^2;
                         valid_sqrt_r = val_r_sq >= 0;
                         r_calc(valid_sqrt_r) = sqrt(val_r_sq(valid_sqrt_r)) - y0;
                         if any(~valid_sqrt_r)
                            % fprintf('경고: Ogive 계산 중 r^2 음수 발생. 해당 값 0 처리.\n');
                         end
                    end
                end
                 % Java의 L < R 스케일링 로직은 입력 처리 단계에서 고려하거나,
                 % 필요하다면 여기에 추가 구현할 수 있음 (현재는 미구현)
            end

        case 'ellipsoid'
            % --- 수정: Java 방식과 동일하게 구현 ---
            % Java: x_scaled = x * R / L; r = sqrt(2*R*x_scaled - x_scaled^2);
            x_scaled = x_calc .* R ./ L; % x 좌표를 먼저 스케일링
            val_under_sqrt = 2 * R .* x_scaled - x_scaled.^2;
            valid_sqrt = val_under_sqrt >= 0;
            r_calc(valid_sqrt) = sqrt(val_under_sqrt(valid_sqrt));
            if any(~valid_sqrt)
                % fprintf('경고: Ellipsoid 계산 중 sqrt 내 음수 발생. 해당 값 0 처리.\n');
            end
            % --- 수정 끝 ---

        case 'power'
            % param 값이 0에 매우 가까울 때 x=0에서 NaN 발생 방지
            if abs(param) < eps && any(abs(x_calc) < eps)
                r_calc(abs(x_calc) < eps) = 0; % x=0 이면 r=0
                valid_pow = abs(x_calc) >= eps;
                if any(valid_pow)
                   r_calc(valid_pow) = R .* (x_calc(valid_pow) ./ L).^param;
                end
            else
                r_calc = R .* (x_calc ./ L).^param;
            end


        case 'parabolicseries'
            if abs(2 - param) < eps % 분모 0 방지
                r_calc = R .* (x_calc ./ L); % 코니컬로 근사
                fprintf('경고: Parabolic Series 계산 중 분모 0 발생 (param=2). 코니컬로 근사.\n');
            else
                r_calc = R .* ((2 .* x_calc ./ L - param .* (x_calc ./ L).^2) ./ (2 - param));
            end

        case 'haack'
            theta = acos(max(-1, min(1, 1 - 2 .* x_calc ./ L))); % acos 입력 범위 제한
            term = theta - sin(2*theta) / 2;
            if abs(param) < eps % LD-Haack (Von Karman)
                val_under_sqrt = term ./ pi;
            else % LV-Haack 등
                val_under_sqrt = (term + param .* sin(theta).^3) ./ pi;
            end
            valid_sqrt = val_under_sqrt >= 0;
            r_calc(valid_sqrt) = R .* sqrt(val_under_sqrt(valid_sqrt));
            if any(~valid_sqrt)
                % fprintf('경고: Haack Series 계산 중 sqrt 내 음수 발생 (param=%.3f). 해당 값 0 처리.\n', param);
            end

        otherwise
            fprintf('경고: 지원하지 않는 형상 "%s". 코니컬로 계산합니다.\n', shape);
            r_calc = R .* x_calc ./ L; % 기본값: 코니컬
    end

    % 계산된 결과를 유효한 인덱스에만 할당
    if isscalar(x)
        r = r_calc;
    else
        r(valid_indices) = r_calc;
    end

catch ME_radius
     fprintf('radiusByShape 오류 (shape=%s, L=%.4g, R=%.4g, param=%.4g): %s\n계산용 x 범위: [%.4g, %.4g]\n', ...
              shape, L, R, param, ME_radius.message, min(x_calc), max(x_calc));
     % 오류 시 0 반환
     if isscalar(x), r = 0; else, r = zeros(size(x)); end
end

% 최종적으로 음수 반경 및 NaN/Inf 방지
r(r < 0 | isnan(r) | isinf(r)) = 0;

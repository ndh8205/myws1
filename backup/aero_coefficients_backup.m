...
% 속도 관련 미분계수 계산 (추가)
[CXu, CYv, CZw, Clp, Cmq, Cnr] = calcVelocityDerivatives(params, alpha, mach, CAm, CYm, CNm, Clpm, Cmqm, Cyawrm);
...
% 모멘트 계수 기울기 계산 함수 (개선된 버전)
function slopes = calculateMomentSlopes(cfd_data, alpha_range, mach_range)
% CFD 데이터로부터 모멘트 계수의 알파 기울기 (dCm/dα) 계산 및 댐핑 미분계수 변환
% NASA 문서 "Missile Aerodynamics for Ascent and Re-entry"의 방법론 적용
...
% NASA 문서에 따른 변환
pitch_scale = -12;  % dCm/dα → Cmqm 변환 계수 (NASA 문서 값 수정)
yaw_scale = -12;    % dCyaw/dα → Cyawrm 변환 계수 (NASA 문서 값 수정)
...
% 속도 미분계수 계산 함수 (자코비안 행렬용 - 추가)
function [CXu, CYv, CZw, Clp, Cmq, Cnr] = calcVelocityDerivatives(params, alpha, mach, CAm, CYm, CNm, Clpm, Cmqm, Cyawrm)
% 속도 관련 미분계수 계산 함수
% 이 함수는 자코비안 행렬에 필요한 속도 미분계수를 계산합니다

    % 받음각 라디안 변환
    alpha_rad = alpha * pi/180;
    
    % 1. X방향 속도 변화에 대한 축력 계수 미분계수 (CXu)
    % 일반적으로 항력 계수에 관련됨
    % 마하수에 따른 효과
    if mach < 0.8
        % 아음속
    ...
    elseif mach < 1.2
        % 천음속 - 항력 급증
        mach_factor_x = 1.0 + 1.5 * (mach - 0.8) / 0.4;
    else
        % 초음속
        mach_factor_x = 2.5;
    end
    
    % X방향 속도 미분계수 (일반적으로 음수)
    base_CXu = -0.01 - 0.05 * abs(CAm);  % 기본 값 + 현재 항력 기반 조정
    CXu = base_CXu * mach_factor_x;
    
    % 2. Y방향 속도 변화에 대한 측력 계수 미분계수 (CYv)
    % 주로 측방향 안정성 관련
    % 받음각 효과
    CYv_alpha_factor = cos(alpha_rad);  % 받음각이 클수록 영향 감소
    
    % 마하수 효과
    if mach < 0.9
        mach_factor_y = 1.0;
    elseif mach < 1.1
        mach_factor_y = 1.0 + 0.2 * (mach - 0.9) / 0.2;  % 천음속 영역에서 소폭 증가
    else
        mach_factor_y = 1.2;
    end
    
    % Y방향 속도 미분계수 (일반적으로 음수 - 안정성 향상)
    base_CYv = -0.15 * (1 - 0.5 * abs(CYm));  % 기본값 조정
    CYv = base_CYv * CYv_alpha_factor * mach_factor_y;
    
    % 3. Z방향 속도 변화에 대한 법선력 계수 미분계수 (CZw)
    % 주로 수직 안정성 관련
    % 받음각 효과
    CZw_alpha_factor = cos(alpha_rad)^2;  % 받음각에 따른 효과
    
    % 마하수 효과
    if mach < 0.9
        mach_factor_z = 1.0;
    elseif mach < 1.1
        mach_factor_z = 1.0 + 0.3 * (mach - 0.9) / 0.2;  % 천음속 영역에서 증가
    else
        mach_factor_z = 1.3;
    end
    
    % Z방향 속도 미분계수 (일반적으로 음수 - 안정성 향상)
    base_CZw = -0.2 - 0.1 * abs(CNm);  % 법선력 기반 조정
    CZw = base_CZw * CZw_alpha_factor * mach_factor_z;
    
    % 4. 롤 속도 변화에 대한 롤 모멘트 미분계수 (Clp)
    % 이미 계산된 Clpm과 관련이 있지만, 자코비안용 스케일링
    Clp = Clpm * 1.1;  % 약간 더 강한 효과 (자코비안 맥락에 맞게)
    
    % 5. 피치 속도 변화에 대한 피치 모멘트 미분계수 (Cmq)
    % 이미 계산된 Cmqm과 관련이 있지만, 자코비안용 스케일링
    Cmq = Cmqm * 1.1;  % 약간 더 강한 효과
    
    % 6. 요 속도 변화에 대한 요 모멘트 미분계수 (Cnr)
    % 이미 계산된 Cyawrm과 관련이 있지만, 자코비안용 스케일링
    Cnr = Cyawrm * 1.1;  % 약간 더 강한 효과
    
    % 미분계수 범위 제한 (물리적으로 타당한 범위로)
    CXu = max(min(CXu, 0), -2.0);    % -2.0 <= CXu <= 0
    CYv = max(min(CYv, 0), -2.0);    % -2.0 <= CYv <= 0
    CZw = max(min(CZw, 0), -2.0);    % -2.0 <= CZw <= 0
    Clp = max(min(Clp, 0), -2.0);    % -2.0 <= Clp <= 0
    Cmq = max(min(Cmq, 0), -15.0);   % -15.0 <= Cmq <= 0
    Cnr = max(min(Cnr, 0), -15.0);   % -15.0 <= Cnr <= 0
end
...
% 평균 기상 조건
params.environment.wind_speed = 0; % [m/s]
params.environment.pressure = 1025; % [hPa]
params.environment.temperature = 29; % [°C]
params.environment.temp_range = [21, 35]; % [°C]

%% 제어기 파라미터 (고정값)
params.control.Ka_pp = [5, 10, 10]'; % Attitude P
params.control.Ka_p = [2, 2, 2]'; % Rate P
params.control.Ka_i = [0, 0, 0]'; % Rate I
params.control.Ka_d = [0.001, 0.01, 0.01]'; % Rate D
...
% MRP 좌표 위치 (노즈콘 기준) - 공력 해석 중심
params.vehicle.MRP_x = 1.61;  % 노즈콘 -> 로켓 노즐방향 [m]
params.vehicle.MRP_y = 0;  % 
params.vehicle.MRP_z = 0;  % 
...
% ==== 카나드 제어 시스템 파라미터 추가 ====
% 카나드 효과 계수 (제어 할당 행렬 B에서 사용하는 값들)
params.vehicle.canard_eps1 = 1.0;  % Z축 모멘트에 대한 카나드 1의 효과
params.vehicle.canard_eps2 = 1.0;  % Y축 모멘트에 대한 카나드 2의 효과
params.vehicle.canard_eps3 = 1.0;  % Z축 모멘트에 대한 카나드 3의 효과
params.vehicle.canard_eps4 = 1.0;  % Y축 모멘트에 대한 카나드 4의 효과

% 카나드 각도 제한 (라디안)
params.vehicle.canard_max_angle = deg2rad(5);  % 최대 ±20도
params.vehicle.canard_min_angle = deg2rad(-5);

% 카나드 기하학적 특성
params.vehicle.canard_area = 0.0012;  % 각 카나드의 면적 [m^2]
params.vehicle.canard_count = 4;    % 카나드 개수

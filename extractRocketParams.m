    ...
    for i = 0:nodeList.getLength-1
        ...
        if node.getNodeType == org.w3c.dom.Node.ELEMENT_NODE
            ...
            currentComponent.Position = struct(...
                ...
                'IsBottomOriented', false); % 추가: bottom 속성 사용 여부
            ...
            % --- 위치 정보 추출 및 절대 위치 계산 ---
            [absolute_position, position_type_desc, isBottomOriented] = getAbsoluteStartPosition_ParamExtract(...
                node, ...
            ...
            switch nodeName
                ...
                case 'bodytube'
                    ...
                    if ~isempty(motorMountNode)
                         ...
                         motorNode = findDirectChildElement_ParamExtract(motorMountNode, 'motor');
                            if ~isempty(motorNode)
                                % motor 태그의 자식 태그에서 정보 추출
                                currentComponent.MotorMount.Motor.Type = getNodeValue_ParamExtract(motorNode, 'type', 'N/A');
                                currentComponent.MotorMount.Motor.Manufacturer = getNodeValue_ParamExtract(motorNode, 'manufacturer', 'N/A');
                                currentComponent.MotorMount.Motor.Designation = getNodeValue_ParamExtract(motorNode, 'designation', 'N/A');
                                currentComponent.MotorMount.Motor.Digest = getNodeValue_ParamExtract(motorNode, 'digest', 'N/A');
                                currentComponent.MotorMount.Motor.Diameter = str2double(getNodeValue_ParamExtract(motorNode, 'diameter', 'NaN'));
                                currentComponent.MotorMount.Motor.Length = str2double(getNodeValue_ParamExtract(motorNode, 'length', 'NaN'));
                                currentComponent.MotorMount.Motor.Delay = getNodeValue_ParamExtract(motorNode, 'delay', 'N/A');
                                
                                % configid는 속성으로 저장되어 있음
                                currentComponent.MotorMount.Motor.ConfigID = char(motorNode.getAttribute('configid'));
                            end
                    end
            ...
            currentComponent.Position.IsBottomOriented = isBottomOriented;  % Bottom 속성 여부 저장

            % Bottom 속성에 따라 시작점과 끝점 계산 방식 변경
            if isBottomOriented
                % bottom 속성이 있는 경우: 끝점이 기준점, 시작점은 끝점 - 길이
                currentComponent.Position.AbsoluteEndX = absolute_position;
                currentComponent.Position.AbsoluteStartX = absolute_position - component_len;
            else
                % 일반적인 경우: 시작점이 기준점, 끝점은 시작점 + 길이
                currentComponent.Position.AbsoluteStartX = absolute_position;
                currentComponent.Position.AbsoluteEndX = absolute_position + component_len;
            end
            ...
            if ~isempty(subcomponentsNode)
                ...
                currentComponent.Subcomponents = processNodeListRecursive_ParamExtract(...
                    ...
                    currentComponent.Position.AbsoluteStartX, ...      % 현재 부품의 절대 시작 위치 -> 하위 부품의 부모 시작
                    component_len, ...         % 현재 부품의 길이 -> 하위 부품의 부모 길이
                    currentComponent.Position.AbsoluteStartX, ...      % 하위 부품의 초기 순차 기준점은 부모의 시작점
            ...
            % 다음 순차 부품은 현재 부품의 끝점 이후에 위치
            next_sequential_x_at_this_level = currentComponent.Position.AbsoluteEndX;
        end % if ELEMENT_NODE
...
function [absolute_position, position_type_desc, isBottomOriented] = getAbsoluteStartPosition_ParamExtract(...
    componentNode, ...
    ...
    absolute_position = sequential_base_x;
    position_type_desc = 'Sequential (default)';
    isBottomOriented = false;  % bottom 속성 여부를 반환하는 플래그
    ...
    try
        ...
        if ~isempty(posList) && posList.getLength > 0
            ...
            if ~isnan(posValue)
                ...
                elseif is_top_level_stage_component
                    ...
                    absolute_position = sequential_base_x; % 이전 형제 끝에 바로 붙임
                    ...
                    switch lower(posType)
                        case 'top'
                            absolute_position = parent_abs_start_x + posValue;
                            position_type_desc = sprintf('Relative to Parent Top (%.4f)', posValue);

                        case 'bottom'
                            % bottom 속성의 경우, 부모의 끝점이 기준점
                            absolute_position = parent_abs_start_x + parent_len + posValue;
                            position_type_desc = sprintf('Relative to Parent Bottom (%.4f)', posValue);
                            isBottomOriented = true;  % bottom 속성 플래그 설정

                        case 'middle'
                            absolute_position = parent_abs_start_x + parent_len / 2 + posValue;
                            position_type_desc = sprintf('Relative to Parent Middle (%.4f)', posValue);

                        case 'sequential'
                            % 순차적 위치 (이전 부품 끝 기준) + 오프셋
                            absolute_position = sequential_base_x + posValue; % 이전 형제 끝 + 오프셋
                            position_type_desc = sprintf('Sequential + Offset (%.4f)', posValue);

                        otherwise
                            % 알 수 없는 타입은 순차적 위치 + 오프셋으로 간주 (기본값 사용)
                            absolute_position = sequential_base_x + posValue;
        ...
        % If no position node exists, the initial absolute_position = sequential_base_x remains.
...
% Helper: 주어진 material 노드에서 density 속성 값을 추출

% % Main Script 
% % HANul 로켓 시뮬레이션 - 쿼터니언 기반 MPC 제어기
...
% 
% % 제어 모드 선택
% control_mode = 2;  % 1: PPID, 2: 쿼터니언 MPC
% controller_names = {'PPID', 'MPC'};
% disp(['선택된 제어 모드: ', controller_names{control_mode}]);
% 
% % 데이터 로깅 변수 초기화
% state_history = zeros(length(X), num_steps);
% state_history2 = zeros(13, num_steps); % 오일러 각 포함 버전
...
% 
%         if mod(i, 400) == 0
%             disp(['시뮬레이션 시간: ', num2str(current_time), ' 초 (', num2str(i/num_steps*100, '%.1f'), '% 완료)']);
%         end
% 
%         % 현재 시간에 따른 파라미터 업데이트
...
%                         % 디버깅: 제어기 호출 전 상태 출력
%                         if mod(i, 100) == 0
%                             disp(['==== 스텝 ', num2str(i), ' (', num2str(current_time), 's) ====']);
%                             disp(['쿼터니언: ', num2str(X(7:10)')]);
%                             disp(['각속도: ', num2str(X(11:13)')]);
%                             disp(['이전 제어 입력: ', num2str(U_prev')]);
%                         end
% 
...
%                         % 디버깅: 제어 입력 출력
%                         if mod(i, 100) == 0
%                             disp(['제어 입력: ', num2str(final_cmd')]);
%                             disp(['계산 시간: ', num2str(comp_time*1000), ' ms']);
%                             disp('==============================');
%                         end
% 
%                         % MPC 디버그 정보 저장
...
%                     catch ME
%                         warning('MPC 제어기 오류: %s', ME.message);
%                         disp(['오류 위치: ', ME.stack(1).name, ' (', num2str(ME.stack(1).line), '행)']);
% 
%                         % 오류 발생 시 PPID로 대체
%                         U = Controller_PPID_Argument(X, Command_Vector, dt_sim, params);
%                         [ final_cmd, debug_cmd ] = Control_Allocator_RCS(U, params);
...
%         % 자세 변환 (쿼터니언 -> 오일러)
%         quat_now = X(7:10);
%         Euler = Quat2Euler(quat_now'); % 행 벡터로 변환
%         X_Euler = X;
...
% % Figure 1: 위치 및 속도
% figure(1);
% sgtitle(['HANul 로켓 시뮬레이션 - ', controller_names{control_mode}, ' 제어기']);
% 
...
% end
% 
% % Figure 2: 자세 및 각속도 - 실제값과 명령값 모두 포함
% figure(2);
...
% 
% % 목표 자세 각도 (롤=0, 피치=88, 요=0 또는 다른 명령값을 사용할 경우 수정)
% target_angles = [0, 88, 0]; % 원하는 목표 각도로 수정
% 
% for k = 1:6
%     subplot(2, 3, k);
% 
...
%             if settling_threshold < 1  % 최소 임계값 설정
%                 settling_threshold = 1;
%             end
% 
%             % 목표값의 ±2% 범위 내에 도달하는 시간
...
%                 line([settling_time settling_time], ylim, 'Color', 'g', 'LineStyle', ':', 'DisplayName', ['정착 시간: ', num2str(settling_time, '%.2f'), 's']);
%             end
%         end
% 
%         % 최대 오버슈트 계산 및 표시 (필요시)
...
% 
%                 if max_overshoot > 0  % 오버슈트가 있는 경우
...
%         end
% 
%         legend({'실제 각도', '목표 각도'}, 'Location', 'best');
%     else  % 각속도
%         plot(t, state_history2(k+6, :), 'b-');
%         % 각속도 목표값이 있다면 추가 (일반적으로 0)
%         hold on;
%         plot(t, zeros(size(t)), 'r--');
%         legend({'실제 각속도', '목표 각속도'}, 'Location', 'best');
%     end
% 
%     xlabel('시간 [s]');
%     ylabel(ylabels{k});
...
% end
% 
% % Figure 3: 제어 입력 (Figure 9로 대체)
% 
% % Figure 4: 공력 및 모멘트
% figure(4);
% sgtitle(['공력 및 모멘트 분석 - ', controller_names{control_mode}, ' 제어기']);
% 
% % 공력 각도
% subplot(2, 2, 1);
% yyaxis left;
...
% grid on;
% 
% % Figure 5: 좌표계 진단 그래프 (PID 코드의 Figure 5)
% figure(5);
% sgtitle('좌표계 진단');
% 
% % 공력 계수 시각화
...
% grid on;
% 
% % Figure 6: moment_coupling 시각화 (PID 코드의 Figure 6)
% figure(6);
% subplot(2,1,1);
% plot(t, moment_coupling_history);
...
% 
% % Plot actual vs target quaternion components - 목표값 강조
% subplot(2,2,1);
...
% end
% 
% title('쿼터니언 성분: 실제 vs 목표');
% xlabel('시간 [s]');
% ylabel('쿼터니언 값');
% legend({'q0 실제', 'q0 목표', 'q1 실제', 'q1 목표', 'q2 실제', 'q2 목표', 'q3 실제', 'q3 목표'}, 'Location', 'best');
% grid on;
% 
% % 쿼터니언 오차 성분
% subplot(2,2,2);
...
% ylabel('오차 크기');
% grid on;
% ylim([0, max(max(quat_error_magnitude), 0.1)]);  % 합리적인 y축 범위 설정
% 
% % Figure 8: 오일러 각도 오차 분석
% figure(8);
% sgtitle(['오일러 각도 오차 분석 - ', controller_names{control_mode}, ' 제어기']);
% 
% % 오일러 각도 오차 계산
...
% line([0 t(end)], [-canard_limit -canard_limit], 'Color', 'r', 'LineStyle', '-.', 'LineWidth', 1);
% text(t(end), canard_limit, ['제한: ±', num2str(canard_limit, '%.1f'), '°'], 'Color', 'r', 'FontWeight', 'bold');
% 
% legend({'카나드 1', '카나드 2', '카나드 3', '카나드 4'});
% grid on;
% 
% % RCS 추력기 제어 입력
% subplot(2, 1, 2);
...
% xlabel('시간 [s]');
% ylabel('RCS 상태 (ON/OFF)');
% title('RCS 추력기 제어');
% legend({'RCS 1', 'RCS 2', 'RCS 3', 'RCS 4'});
% ylim([-0.1, 1.1]);
% grid on;
% 
% % Figure 10 (새로 추가): MPC 특화 그래프 - 제어 목표 시간 및 예측 궤적
% figure(10);
% sgtitle(['MPC 성능 분석 - ', controller_names{control_mode}, ' 제어기']);
% 
% % MPC 목표 시간과 실제 계산 시간 비교
% target_time = (1/attitude_Control_Hz) * 1000; % 목표 제어 주기 (ms)
% plot(t, mpc_computation_time * 1000, 'b-', 'LineWidth', 1.5); % 실제 계산 시간 (ms)
% hold on;
% plot(t, target_time * ones(size(t)), 'r--', 'LineWidth', 1.5); % 목표 시간
% 
% % 중요: 목표 계산 시간 제한을 명확하게 표시
% text(t(end)*0.05, target_time*1.1, ['목표 계산 시간: ' num2str(target_time, '%.1f') ' ms'], 'Color', 'r', 'FontWeight', 'bold');
% ylabel('계산 시간 [ms]');
% xlabel('시뮬레이션 시간 [s]');
% title(['MPC 제어기 계산 제한 시간 (' num2str(target_time, '%.1f') ' ms) vs 실제 계산 시간']);
% legend('실제 계산 시간', '제한 시간');
% grid on;
% 
% % 계산 시간이 목표를 초과하는 부분을 빨간색으로 강조
...
%     legend('실제 계산 시간', '제한 시간', '제한 시간 초과');
% end
% 
% %% 성능 지표 계산 및 출력
% 
...
% for i = 1:3
%     % 롤, 요: 목표는 0도, 피치: 목표는 88도 부근
%     target_val = euler_command_history(i, end);
% 
%     threshold = abs(target_val) * 0.02;  % 목표의 ±2%
%     if threshold < 2  % 최소 임계값 설정 (2도)
%         threshold = 2;
%     end
% 
...
% 
% fprintf('1. 자세 제어 정확도 (RMSE):\n');
% fprintf('   롤: %.2f deg\n', rmse_attitude(1));
% fprintf('   피치: %.2f deg\n', rmse_attitude(2));
% fprintf('   요: %.2f deg\n\n', rmse_attitude(3));
% 
% fprintf('2. 각속도 안정성 (RMSE):\n');
% fprintf('   롤 각속도: %.4f rad/s\n', omega_rmse(1));
% fprintf('   피치 각속도: %.4f rad/s\n', omega_rmse(2));
% fprintf('   요 각속도: %.4f rad/s\n\n', omega_rmse(3));
% 
% fprintf('3. 제어 입력 사용량:\n');
% fprintf('   카나드 평균 사용량: %.2f deg\n', rad2deg(mean(canard_usage)));
% fprintf('   RCS 추력기 평균 사용률: %.2f%%\n\n', mean(rcs_usage)*100);
% 
% fprintf('4. 자세 정착 시간:\n');
...
%     fprintf('   최대 고도 도달 시간: %.2f 초\n', apogee_time);
% end
% 
% fprintf('=======================================\n');

% Main Script 
% HANul Rocket Simulation - Quaternion-based MPC Controller
% Last update: 2025-05-03

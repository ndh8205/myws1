...
addpath(genpath('C:\Users\USER\Desktop\SAT_new'));
...
U = [0 0 0 0 0 0 0 0 49]'; % ( T1 T2 T3 T4 T5 T6 T7 T8 RW )
...
for i = 1:numSteps
    ...
    [ U , computation_time, predicted_trajectory] = Controller_NMPC_CAN_v2(X, X_target, command, max_torque, idle, dt, params, XF);
    ...
    % Motor_scale_save(:, i) = LIMIT2( motor_command, 0, 98 );
    %%
...
figure(4);
k2=0.0625:0.0625:simulationEndTime;
for i = 1:8
    subplot(8, 1, i);
    stairs(k2,debug_control(i,:))
    % ylim([0, 1]);
    xlabel('[sec]')
    ylabel('[ON-OFF]')
...
xlabel('X Position [mm]');
ylabel('Y Position [mm]');
title('Actual Trajectory and Predicted Trajectories');

...
global mission_state;  % 액추에이터 제어용 mission state
mission_state = 0;
...
hwobj = params.System.hwjetson1;

% Socket communication
try
    client = tcpclient(params.System.host1, params.System.commandPort, 'Timeout', 30);
...
docking_completed = false; % 도킹 완료 플래그
...
while true
    ...
    % 도킹 완료 시 액추에이터 작동
    if msp == 3 && ~docking_completed
        disp('Docking completed! Turning off thrusters and activating actuator...');
        
        % 추력기 모두 OFF (중요!)
        relay_commands = ones(8,1);
        ...
        sendRelayCommand(client, relay_commands);
        sendRelayCommand(client, relay_commands);
        pause(0.5);  % 추력기 OFF 확실히 전달
        
        % 액추에이터 작동
        mission_state = 12;  % GPIO 38 작동 (Mission 6으로 설정)
        sendMissionCommand(client, mission_state);
        docking_completed = true;
        
        % 액추에이터 작동 후 일정 시간 후 종료
        pause(15);  % 액추에이터 작동 대기
        
        % 프로그램 종료 전 모든 것 OFF
        mission_state = 6;
        sendRelayCommand(client, ones(8,1));
        sendMissionCommand(client, mission_state);
        break;
    end
    ...
    if stopFlag
        ...
        sendMissionCommand(client, 0); % Mission state 초기화
...
% 저장 폴더 생성
save_folder = fullfile(pwd, 'test_results', ['docking_' timestamp]);
if ~exist(save_folder, 'dir')
    mkdir(save_folder);
end

% 데이터 저장
save_filename = fullfile(save_folder, ['docking_data_' timestamp '.mat']);

% 모든 중요 데이터 저장
save(save_filename, ...
    't', ...                    % 시간 벡터
    'state_history', ...        % 상태 이력
    'X_target_history', ...     % 목표 상태 이력
    'X_raw', ...               % 원시 데이터
    'debug_control', ...        % 릴레이 제어 이력
    'msp_history', ...         % 미션 상태 이력
    'alignment_point', ...      % 정렬 시작 지점
    'approach_point_history', ... % 접근 포인트
    'params');                  % 파라미터 구조체
...
%% Mission Command Function (추가)
function sendMissionCommand(client, mission_state)
    % Mission state 명령을 'M' 프리픽스와 함께 전송
    command = sprintf('M %d', mission_state);
    write(client, uint8(command));
end